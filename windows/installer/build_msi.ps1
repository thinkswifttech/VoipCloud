...
if ($productionValues["APP_ENV"] -ne "production") {
  throw "config\production.json must set APP_ENV to production."
}
...
# SUBST. Keep the SDK source ready, but copy it only after Flutter's configure
# pass because that pass recreates the ephemeral directory.
...
try {
  ...
  $mappedProductionConfig = Join-Path $buildProjectRoot `
    "config\production.json"
  & $flutter build windows --release --no-pub --config-only `
    "--dart-define-from-file=$mappedProductionConfig"
  if ($LASTEXITCODE -ne 0) {
    throw "Flutter Windows release configuration failed."
  }
  ...
  $generatedConfig = Join-Path $buildProjectRoot `
    "windows\flutter\ephemeral\generated_config.cmake"
  if (-not (Test-Path -LiteralPath $generatedConfig)) {
    throw "Flutter did not generate its Windows configuration."
  }
  ...
  foreach ($entry in $productionValues.GetEnumerator()) {
    ...
    $encodedDefine = [Convert]::ToBase64String(
      [Text.Encoding]::UTF8.GetBytes($define)
    )
    if (-not $generatedConfigText.Contains($encodedDefine)) {
      throw "Flutter omitted production define '$($entry.Key)'."
    }
  }
  Write-Host "Verified config\production.json in generated Windows defines."

  $wrapperTarget = Join-Path $buildProjectRoot `
  ...
  & $cmake --build $cmakeBuildDirectory --config $Configuration `
    --target INSTALL
